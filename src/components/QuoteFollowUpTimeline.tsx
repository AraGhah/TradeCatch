...
export function QuoteFollowUpTimeline({
  ...
  return (
    <div className="rounded-[18px] border border-[rgba(12,20,30,0.1)] bg-white p-5 sm:p-6">
        ...
        {steps.map((step, i) => (
          <li key={`${step.day}-${step.title}`} className="relative flex gap-4 pb-6 last:pb-0">
            {i < steps.length - 1 ? (
              <span
                aria-hidden
                className="absolute top-7 left-[11px] h-[calc(100%-1.25rem)] w-px bg-[rgba(12,20,30,0.12)]"
              />
            ) : null}
            ...
            <div className="min-w-0 pt-0.5">
              ...
              <p className="mt-1 text-[15px] font-semibold text-navy">{step.title}</p>
